...
WAN_FETCH_GRACE    = 0.4   # [MISC] was a magic number inline (+0.4)
PING_COUNT         = 2
PING_TIMEOUT_S     = 1.5
DNS_CACHE_TTL_S    = 30   # [CACHE] seconds before a cached DNS entry expires

# [SEC-2] Strict FQDN: labels.tld with TLD >= 2 chars; rejects bare IPs, localhost
_FQDN_RE = re.compile(
    r'^(?:[a-zA-Z0-9](?:[a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}$'
)
# [BUG-3] Valid saved geometry string pattern
...
def _is_public_ip(value: str) -> bool:
    """[SEC-1] True only for syntactically valid, globally-routable IPv4."""
    try:
...
class _DnsCacheEntry:
    """[CACHE] Cached DNS result with TTL expiry."""
    addrs:      list
    expires_at: float = field(default_factory=lambda: time.monotonic() + DNS_CACHE_TTL_S)
...
def _make_session() -> requests.Session:
    s = requests.Session()
    s.verify = True  # explicit; default but documented
    retry = Retry(total=1, backoff_factor=0.15, status_forcelist=[500, 502, 503, 504])
    # [MISC] pool_maxsize raised 10 -> 12 to give headroom for 10 workers
...
class IPMonitorApp(ctk.CTk):
    def __init__(self) -> None:
        ...
        # [CACHE] Site-level DNS cache keyed by hostname
        ...
        # [SEC-4] Validate last_wan_ip from config before trusting it
        raw_last_ip = self._raw_cfg.get("last_wan_ip")
        self.last_logged_ip: Optional[str] = (
            raw_last_ip if isinstance(raw_last_ip, str) and _is_public_ip(raw_last_ip) else None
        )
        ...
        # [BUG-3] Validate geometry string before applying to prevent off-screen window
        geo = self._raw_cfg.get("geometry")
        if geo and isinstance(geo, str) and _GEOMETRY_RE.match(geo):
            try:
                self.geometry(geo)
            except Exception:
                pass
    ...
    def _load_raw_config(self) -> dict:
        ...
        try:
            ...
            # [BUG-4] Log to stderr since file logger is not ready yet
            print(f"[WARN] Could not load config ({self.config_file}): {exc}", file=sys.stderr, flush=True)
            return {}
    ...
    def build_ui(self) -> None:
        ...
        ctk.CTkLabel(title_wrap, text=f"dark • green • trader-grade  •  v{APP_VERSION}", font=ctk.CTkFont(size=12, slant="italic"), text_color="#9dd6b3").pack(anchor="w")
    ...
    def build_right_panel(self) -> None:
        ...
        self._explain_label = ctk.CTkLabel(
            explain,
            text="DNS = resolver  |  TLS = actual HTTPS peer  |  Seen As = your public IP  |  HTTP-only = no TLS",
    ...
    def resolve_selected(self) -> None:
        ...
        # [BUG-2] Submit to executor instead of spawning unbounded raw threads
        self.executor.submit(self._resolve_single_worker, site)
    ...
    def _get_wan_ip_parallel(self) -> str:
        """[BUG-1][SEC-1] Race-safe WAN IP fetch with proper IP validation."""
        result_holder: list = [None]
        done = threading.Event()
        lock = threading.Lock()  # [BUG-1] guards the TOCTOU on done + result_holder

        def fetch(url: str) -> None:
            if done.is_set():
                return
            try:
                r = self.http_session.get(url, timeout=WAN_FETCH_TIMEOUT, verify=True)
                if r.ok:
                    val = r.text.strip()
                    # [SEC-1] Validate via ipaddress; rejects non-global/malformed IPs
        ...
        for t in threads:
            t.start()
        done.wait(timeout=WAN_FETCH_TIMEOUT + WAN_FETCH_GRACE)  # [MISC] named constant
        return result_holder[0] or "Unavailable"

    def _cached_dns(self, site: str) -> list:
        """[CACHE] Return cached IPv4 addresses for site, refreshing if stale."""
        with self._dns_cache_lock:
            entry = self._dns_cache.get(site)
            if entry and entry.is_valid():
                return entry.addrs
        # Cache miss or expired — live lookup outside the lock
        addrinfo = socket.getaddrinfo(site, 443, family=socket.AF_INET, type=socket.SOCK_STREAM)
        addrs = sorted({item[4][0] for item in addrinfo})
        with self._dns_cache_lock:
            self._dns_cache[site] = _DnsCacheEntry(addrs=addrs)
        return addrs
    ...
    def _resolve_site_parallel(self, site: str, seen_as_ip: str) -> SiteResult:
        ...
        try:
            # [CACHE] Use the TTL-aware DNS cache
            ipv4s = self._cached_dns(site)
    ...
    def _get_tls_peer_ip(self, host: str) -> tuple:
        """[SEC-5] HTTPS attempted first; plaintext fallback labelled 'HTTP-only'."""
        # Try HTTPS (port 443) first
        try:
            start = time.perf_counter()
            raw = socket.create_connection((host, 443), timeout=REFRESH_TIMEOUT)
            ctx = ssl.create_default_context()
            with ctx.wrap_socket(raw, server_hostname=host) as tls:
                peer_ip = tls.getpeername()[0]
            elapsed = f"{(time.perf_counter() - start) * 1000:.0f}"
            return peer_ip, elapsed, "OK"
        except Exception:
            pass

        # Plaintext fallback — clearly labelled so the UI is not misleading
        try:
            start = time.perf_counter()
            raw = socket.create_connection((host, 80), timeout=REFRESH_TIMEOUT)
            with raw:
                peer_ip = raw.getpeername()[0]
            elapsed = f"{(time.perf_counter() - start) * 1000:.0f}"
            return peer_ip, elapsed, "HTTP-only"
        except Exception:
            pass
    ...
    def _append_wan_log(self, current_ip: str) -> tuple:
        """[SEC-3] Sanitise IP before writing to CSV to prevent formula injection."""
        ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        changed = self.last_logged_ip is not None and current_ip != self.last_logged_ip
        # [SEC-3] Accept only valid public IPs in the log; flag anything else
        safe_ip = current_ip if _is_public_ip(current_ip) else f"INVALID:{current_ip[:20]}"
        # Atomic-ish header: check existence before open to avoid TOCTOU race
        write_header = not self.wan_log_file.exists()
        try:
            with self.wan_log_file.open("a", newline="", encoding="utf-8") as f:
                w = csv.writer(f)
                if write_header:
                    w.writerow(["Timestamp", "WAN_IP", "Changed"])
                w.writerow([ts, safe_ip, "Yes" if changed else "No"])
        except PermissionError as exc:
            # Another instance may be writing; log and skip this entry
            self.ui_queue.put(("log", (f"WAN log write skipped (locked): {exc}", "WARN")))
            return ts, changed
    ...
    def _row_data(self, site: str, r: Optional[SiteResult]) -> tuple:
        if r is None:
            return (site, "-", "-", self.wan_ip, "-", "-", "-", "-", "-", "Pending"), "pending"
        # [SEC-5] 'HTTP-only' is a distinct note; map it to 'warn' not 'bad'
        note_lower = r.note.lower()
        if r.note == "OK":
            tag = "ok"
        elif any(k in note_lower for k in ("fail", "error", "connect")):
            tag = "bad"
        else:
            tag = "warn"
        return (
    ...
    def open_edit_sites_dialog(self) -> None:
        ...
        ctk.CTkLabel(dialog, text="One domain per line  •  # comments ignored  •  must be valid FQDNs", text_color="#9dd6b3").pack(pady=(12, 6))
    ...
    def add_site(self) -> None:
        ...
        if not site:
            self.set_status("Enter a valid FQDN (e.g. broker.com)", "#ffd27d")
            return
        ...
        # [BUG-2] Use executor instead of raw threading.Thread
        self.executor.submit(self._resolve_single_worker, site)
    ...
    def remove_selected_site(self) -> None:
        ...
        # Evict stale DNS cache entry for removed site
        with self._dns_cache_lock:
            self._dns_cache.pop(site, None)
        self.save_config()
        self.refresh_site_listbox()
        self.render_table()
        self.refresh_stat_cards()
    ...
    def normalize_site(self, value: str) -> str:
        """[SEC-2] Strip scheme/path then enforce strict FQDN validation."""
        if not value:
            return ""
        v = value.replace("https://", "").replace("http://", "").strip().strip("/")
        v = v.split("/")[0].strip().lower()
        # Reject anything that isn't a well-formed multi-label domain name
        return v if _FQDN_RE.match(v) else ""
