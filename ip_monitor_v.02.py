...
# Use icmplib for fast, native Python pinging (no subprocess overhead)
...
class IPMonitorApp(ctk.CTk):
    ...
    def _setup_file_logger(self) -> None:
        ...
        if not self._flogger.handlers:
            h = logging.handlers.RotatingFileHandler(
                self.action_log_file, maxBytes=MAX_LOG_FILE_BYTES, backupCount=3, encoding="utf-8",
            )
    ...
    def _parse_sites(self, cfg: dict) -> list[str]:
        ...
        if isinstance(raw, list) and raw:
            ...
            for s in raw:
                ...
                if n and n not in cleaned: cleaned.append(n)
    ...
    def save_config(self) -> None:
        try:
            with self.config_file.open("w", encoding="utf-8") as f:
                json.dump({
                    "sites": self.websites, "auto_refresh": self.auto_refresh_enabled.get(),
                    "interval": self.get_interval_seconds(), "last_wan_ip": self.last_logged_ip,
    ...
    def build_ui(self) -> None:
        ...
        ctk.CTkLabel(title_wrap, text=APP_TITLE, font=ctk.CTkFont(size=30, weight="bold"), text_color="#e8fff1").pack(anchor="w")
        ctk.CTkLabel(title_wrap, text="dark • green • trader-grade monitoring", font=ctk.CTkFont(size=13, slant="italic"), text_color="#9dd6b3").pack(anchor="w", pady=(2, 0))

        header_actions = ctk.CTkFrame(header, fg_color="transparent")
        header_actions.grid(row=0, column=1, sticky="e", padx=18, pady=14)

        self.status_chip = ctk.CTkLabel(header_actions, text="Ready", height=34, corner_radius=999, padx=14, fg_color="#143a24", text_color="#84f0b1", font=ctk.CTkFont(size=13, weight="bold"))
        self.status_chip.pack(side="left", padx=(0, 10))

        ctk.CTkButton(header_actions, text="Refresh Now", width=120, height=38, command=self.refresh_all, fg_color="#1f8f55", hover_color="#166c40").pack(side="left", padx=5)
        ctk.CTkButton(header_actions, text="Exit", width=90, height=38, command=self.on_exit, fg_color="#7a1f1f", hover_color="#5c1717").pack(side="left", padx=(5, 0))
    ...
    def build_left_panel(self) -> None:
        ...
        ctk.CTkLabel(controls, text="Auto Refresh", font=ctk.CTkFont(size=16, weight="bold")).pack(anchor="w", padx=14, pady=(12, 8))

        switch_row = ctk.CTkFrame(controls, fg_color="transparent")
        switch_row.pack(fill="x", padx=14, pady=(0, 8))
        self.auto_switch = ctk.CTkSwitch(switch_row, text="Enable auto refresh + WAN logging", variable=self.auto_refresh_enabled, onvalue=True, offvalue=False, command=self.on_auto_toggle, progress_color="#1f8f55")
        self.auto_switch.pack(anchor="w")
        if self.auto_refresh_enabled.get(): self.auto_switch.select()
        ...
        self.interval_menu = ctk.CTkOptionMenu(interval_row, values=["5", "10", "15", "30", "60"], variable=self.interval_var, command=self.on_interval_change, fg_color="#17452a", button_color="#1f8f55", button_hover_color="#166c40")
        self.interval_menu.pack(side="left", padx=(0, 8))
        ctk.CTkLabel(interval_row, text="seconds", text_color="#99b7a4").pack(side="left")

        self.countdown_label = ctk.CTkLabel(controls, text=f"Next refresh in {self.countdown_seconds}s", font=ctk.CTkFont(size=14, weight="bold"), text_color="#84f0b1")
        ...
        for lbl in (self.refresh_meta_label, self.wan_change_label, self.wan_change_time_label): lbl.pack(fill="x", padx=14, pady=2)
        ...
        ctk.CTkButton(sites_header, text="✏️ Edit List", width=90, height=20, command=self.open_edit_sites_dialog, fg_color="#2a5f3d", hover_color="#1f8f55", font=ctk.CTkFont(size=11)).pack(side="right")

        ctk.CTkLabel(sites, text="• Double-click to resolve • ↑↓ to reorder", text_color="#97b7a2", font=ctk.CTkFont(size=10)).pack(anchor="w", padx=14, pady=(0, 6))

        self.site_listbox = tk.Listbox(sites, height=8, font=("Consolas", 11), bg="#0f1511", fg="#dcfce7", bd=0, highlightthickness=0, relief="flat", selectbackground="#1f8f55", selectforeground="#ffffff", exportselection=False)
        ...
        ctk.CTkButton(entry_row, text="Add", width=70, command=self.add_site, fg_color="#1f8f55", hover_color="#166c40").pack(side="left")
        ...
        ctk.CTkButton(manage_row, text="Remove", width=80, fg_color="#6f2b2b", hover_color="#572121", command=self.remove_selected_site).pack(side="left", padx=(4, 0))
    ...
    def _metric_block(self, parent, label: str, value: str, color: str):
        ...
        ctk.CTkLabel(wrap, text=label, text_color="#98b7a4", font=ctk.CTkFont(size=13, weight="bold")).pack(anchor="w")
        lbl = ctk.CTkLabel(wrap, text=value, font=ctk.CTkFont(size=20, weight="bold", family="Consolas"), text_color=color, anchor="w", justify="left")
    ...
    def build_right_panel(self) -> None:
        ...
        for i in range(4): top_cards.grid_columnconfigure(i, weight=1)
        ...
        ctk.CTkLabel(explain, text="DNS IP vs TLS IP", font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", padx=14, pady=(10, 4))
        self._explain_label = ctk.CTkLabel(explain, text="DNS IP = address from resolver  |  TLS IP = server your HTTPS actually reached  |  Seen As = your WAN IP (what APIs log)", justify="left", text_color="#c5dfce", font=ctk.CTkFont(size=11), anchor="w")
        ...
        headings = {"Site": "Website", "DNS_IP": "DNS IP", "TLS_IP": "TLS IP", "Seen_As": "Server Sees You As", "DNS_ms": "DNS ms", "Connect_ms": "TLS ms", "Min": "Ping Min", "Max": "Ping Max", "Avg": "Ping Avg", "Status": "Status"}
        widths = {"Site": 160, "DNS_IP": 145, "TLS_IP": 125, "Seen_As": 135, "DNS_ms": 70, "Connect_ms": 70, "Min": 65, "Max": 65, "Avg": 65, "Status": 165}
        for col in columns:
        ...
        ctk.CTkLabel(graph_hdr, text="📈 WAN IP History", font=ctk.CTkFont(size=16, weight="bold")).grid(row=0, column=0, sticky="w")
        self.graph_status_label = ctk.CTkLabel(graph_hdr, text="Waiting for data…", text_color="#9dd6b3", font=ctk.CTkFont(size=10))
        ...
        ctk.CTkLabel(log_hdr, text="📋 Activity Log", font=ctk.CTkFont(size=16, weight="bold")).grid(row=0, column=0, sticky="w")
        ctk.CTkButton(log_hdr, text="Clear", width=80, height=26, command=self.clear_log_view, fg_color="#3a3a3a", hover_color="#4a4a4a", font=ctk.CTkFont(size=10)).grid(row=0, column=1, sticky="e")

        self.log_text = tk.Text(log_wrap, font=("Consolas", 10), bg="#0d120f", fg="#e2ffe9", insertbackground="#9dffbf", bd=0, padx=12, pady=8, wrap="none", relief="flat", highlightthickness=0)
    ...
    def _small_stat_card(self, parent, col: int, label: str, value: str, color: str):
        ...
        v_lbl = ctk.CTkLabel(card, text=value, font=ctk.CTkFont(size=24, weight="bold"), text_color=color)
    ...
    def configure_tree_style(self) -> None:
        ...
        s.configure("Treeview", rowheight=30, font=("Segoe UI", 10), background="#0d120f", fieldbackground="#0d120f", foreground="#e2ffe9", borderwidth=0)
        s.configure("Treeview.Heading", font=("Segoe UI", 10, "bold"), background="#1b2a20", foreground="#e2ffe9")
    ...
    def get_interval_seconds(self) -> int:
        try: return max(5, int(self.interval_var.get().strip()))
        except Exception: return DEFAULT_INTERVAL

    def _nudge_queue(self) -> None: self.after(0, self._drain_queue)

    def _queue_flush_tick(self) -> None:
        self._drain_queue()
        if not self.stop_event.is_set(): self.after(100, self._queue_flush_tick)
    ...
    def _handle_ui_event(self, event: str, payload) -> None:
        if event == "status":
            ...
            self.log_action(f"{result.site} | DNS={result.dns_ip} | TLS={result.tls_ip} | Ping={result.ping_min}/{result.ping_avg}/{result.ping_max}ms | {result.note}", "RESULT")
    ...
    def refresh_all(self) -> None:
        if self.refresh_in_progress: return
    ...
    def _refresh_worker(self) -> None:
        try:
            ...
            futures = {self.executor.submit(self._resolve_site_parallel, site, wan_ip): site for site in self.websites}
    ...
    def _resolve_site_parallel(self, site: str, seen_as_ip: str) -> SiteResult:
        ...
        def do_tls(): tls_result[0] = self._get_tls_peer_ip(site)
        def do_ping():
            if primary_ip: ping_result[0] = self._ping_host(primary_ip)
        ...
        for port, secure in ((443, True), (80, False)):
            try:
                ...
                if secure:
                    ...
                    with ctx.wrap_socket(raw, server_hostname=host) as tls: peer_ip = tls.getpeername()[0]
                else:
                    with raw: peer_ip = raw.getpeername()[0]
    ...
    def _ping_host(self, host: str, count: int = PING_COUNT) -> Optional[dict[str, str]]:
        # Use icmplib instead of subprocess for massive speed improvements
        try:
            result = ping(host, count=count, timeout=PING_TIMEOUT_S, privileged=False)
            if result.is_alive:
                return {"min": f"{result.min_rtt:.1f}", "max": f"{result.max_rtt:.1f}", "avg": f"{result.avg_rtt:.1f}"}
            return None
        except Exception:
            return None
    ...
    def _append_wan_log(self, current_ip: str) -> tuple[str, bool]:
        ...
        with self.wan_log_file.open("a", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            if not exists: w.writerow(["Timestamp", "WAN_IP", "Changed"])
    ...
    def _on_wan_logged(self, current_ip: str, changed: bool, ts: str) -> None:
        ...
        self.refresh_summary_labels()
        if changed: self.log_action(f"WAN IP changed → {current_ip} at {ts}", "WARN")
        else: self.log_action(f"WAN IP logged: {current_ip}", "INFO")
    ...
    def _push_graph_point(self, ip: str, y_val: float) -> None:
        # Only append if it's a real point (fixes bug where priming adds dummy 0.0 points)
        if ip: 
        ...
        # Use draw_idle() to queue the redraw and prevent blocking the main UI thread
        self._graph_canvas.draw_idle()   
        n_unique = len(uniq_ips)
        self.graph_status_label.configure(text=f"{len(xs)} entries • {n_unique} unique IP{'s' if n_unique != 1 else ''}")

    def _load_wan_history_meta_and_prime_graph(self) -> None:
        if not self.wan_log_file.exists(): return
        try:
            # Replaced Pandas with built-in csv for massive startup speed/memory improvements
            ...
            self.last_wan_change_at = last_change_ts if rows else "-"
            if self.last_logged_ip is None: self.last_logged_ip = last_ip

            recent = rows[-MAX_GRAPH_POINTS:]
            for i, row in enumerate(recent, start=1):
                ip = (row.get("WAN_IP") or "").strip()
                if not ip: continue
                if ip not in self._ip_index_map: self._ip_index_map[ip] = len(self._ip_index_map) + 1
            ...
            if self._graph_values:
                self.after(400, lambda: self._push_graph_point("", 0.0))
        except Exception as e:
    ...
    def _update_table_row(self, result: SiteResult) -> None:
        ...
        if self.tree.exists(result.site): self.tree.item(result.site, values=values, tags=(tag,))
        else: self.render_table()
        self._apply_tree_tags()

    def _row_data(self, site: str, r: Optional[SiteResult]) -> tuple[tuple, str]:
        if r is None: return (site, "-", "-", self.wan_ip, "-", "-", "-", "-", "-", "Pending"), "pending"
        tag = "ok" if r.note == "OK" else "bad" if any(k in r.note.lower() for k in ("failed", "error", "connect")) else "warn"
        return (r.site, r.dns_ip, r.tls_ip, r.seen_as, r.dns_ms, r.connect_ms, r.ping_min, r.ping_max, r.ping_avg, r.note), tag
    ...
    def log_action(self, message: str, level: str = "INFO") -> None:
        ...
        # Optimized state toggling to prevent UI micro-stutters
        is_disabled = str(self.log_text.cget("state")) == "disabled"
        if is_disabled: self.log_text.configure(state="normal")
        ...
        if is_disabled: self.log_text.configure(state="disabled")
    ...
    def open_edit_sites_dialog(self) -> None:
        ...
        ctk.CTkLabel(dialog, text="One domain per line  •  # lines are ignored", text_color="#9dd6b3").pack(pady=(15, 8))
        tf = ctk.CTkFrame(dialog, fg_color="#0f1511", corner_radius=10)
        tf.pack(fill=tk.BOTH, expand=True, padx=20, pady=5)
        text_w = scrolledtext.ScrolledText(tf, font=("Consolas", 10), bg="#0d120f", fg="#dcfce7", insertbackground="#9dffbf", bd=0, padx=10, pady=8, wrap="none")
        ...
        ctk.CTkButton(btn_f, text="Cancel", width=90, command=dialog.destroy, fg_color="#3a3a3a", hover_color="#4a4a4a").pack(side=tk.RIGHT, padx=5)
        ctk.CTkButton(btn_f, text="Save Changes", width=110, command=save_and_close, fg_color="#1f8f55", hover_color="#166c40").pack(side=tk.RIGHT, padx=5)
        text_w.focus_set()

    def on_site_select(self, event=None) -> None:
        sel = self.site_listbox.curselection()
        if sel: self.set_status(f"Selected: {self.site_listbox.get(sel[0])}", "#b6f59d")

    def refresh_site_listbox(self) -> None:
        self.site_listbox.delete(0, "end")
        for s in self.websites: self.site_listbox.insert("end", s)
